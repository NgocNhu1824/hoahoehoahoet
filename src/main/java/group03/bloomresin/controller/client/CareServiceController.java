...
public class CareServiceController {
    ...
    public Map<String, String> processChatApi(@RequestBody Map<String, String> request) {
        ...
        // Priority 1: Instant 100% accurate store facts for store topics (contact info, address, phone, custom order)
        String storeFact = getExactStoreFact(userMessage);
        if (storeFact != null) {
            response.put("reply", storeFact);
            return response;
        }

        // Priority 2: External AI call with full store context prompt
        String aiReply = callRealAiApi(userMessage);
        response.put("reply", aiReply);
        return response;
    }

    private String getExactStoreFact(String query) {
    ...
    private String callRealAiApi(String userMessage) {
        try {
            ...
            String systemPrompt = "Bạn là Trợ lý AI thông minh của BloomResin (Thương hiệu trang sức hoa tự nhiên ép khô). Thông tin cửa hàng chính thức:\n" +
                "- Địa chỉ: FPT University, 600 Nguyen Van Cu, An Binh, Ninh Kieu, Can Tho\n" +
                "- Hotline/Zalo: 0989780481\n" +
                "- Email: bloomresin.system@gmail.com\n" +
                "- Giờ mở cửa: 08:00 - 21:30 (Tất cả các ngày trong tuần)\n" +
                "Trả lời bằng tiếng Việt ngắn gọn, lịch sự, chính xác.";
